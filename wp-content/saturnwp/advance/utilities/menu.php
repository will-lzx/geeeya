...
if(!function_exists('saturnwp_add_menu_parent_class')){
	/**
	 ...
	 * @return array
	 */
	function saturnwp_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'saturnwp_add_menu_parent_class' );


if(!function_exists('saturnwp_page_menu')){
	/**
	 ...
	 */
	function saturnwp_page_menu($only_check = false) {
        global $post;
        ...
        if($there_is_menu){
			...
			get_template_part('advance/walkers/children', 'pages'); 

            $list_pages_params['walker'] = new SATURNWP_list_pages_walker;
