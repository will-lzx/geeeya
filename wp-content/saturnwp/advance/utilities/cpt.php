<?php
if(!function_exists( 'saturnwp_cpt_meta_fields' )){
	/**
	 * Return internet address and custom fields of post
	 *
	 * @return string
	 */
    function saturnwp_cpt_meta_fields(){
        $fields = '';
	    $custom_fields_number = apply_filters('saturnwp_custom_fields_number', 5);
        ...
        for($i = 0; $i <= $custom_fields_number; $i++){
	        if( $i === 0 ) {
		        ...
		        if ( strlen( $temp ) ) {
			        $temp = esc_html__( 'Website', 'saturnwp' ).':'.$temp;
		        }
	        }
	        else{
		        //custom fields
		...
		if(strlen($fields)){
			$html =
				'<div class="meta-data">'
			       ...
			       .apply_filters('saturnwp_after_cpt_meta_fields','').
		        '</div>';
		}
...
if(!function_exists( 'saturnwp_cpt_meta_data' )){
	/**
	 ...
	 * @return string
	 */
    function saturnwp_cpt_meta_data( $content ){
	    return $content . saturnwp_cpt_meta_fields();
    }
}


if(!function_exists( 'saturnwp_cpt_social' )){
	/**
	 ...
	 *
	 */
    function saturnwp_cpt_social( $link, $title ){
 ...
 */
function saturnwp_get_gallery_item_images($item, $collector, $columns, $bricks_max_width, $brick_margin, $brick_proportion = 0){
	global $saturnwp_a13;
	...
	$quality = (int)$saturnwp_a13->get_option( 'a13ir_image_quality' );
...
if(!function_exists('saturnwp_cpt_list_navigation')){
	/**
	 * Displays page navigation for custom post types
	 *
	 * @param bool|WP_Query $query
	 */
	function saturnwp_cpt_list_navigation( $query = false ){
		if($query === false){
			global $wp_query;
			$query = $wp_query;
		}
		...
		$links = paginate_links( apply_filters( 'saturnwp_pagination_args', array(
			'add_args'     => '',
...
if(!function_exists('saturnwp_loop_pagination')){
	/**
	 ...
	 * @return array
	 */
	function saturnwp_loop_pagination($args){
		$args['type'] = 'plain';
		$args['prev_text'] = _x( 'Previous', 'previous product', 'saturnwp' );
		$args['next_text'] = _x( 'Next', 'next product', 'saturnwp' );

		return $args;
	}
}
add_filter('saturnwp_pagination_args', 'saturnwp_loop_pagination' );


if(!function_exists('saturnwp_make_media_collection')){
	/**
	 ...
	 */
    function saturnwp_make_media_collection($id = null, $args = array()){
	    if(is_null($id)){
	        $id = get_the_ID();
	    }
	    ...
	    <ul class="gallery-media-collection screen-reader-text">
			<?php echo wp_kses_post(saturnwp_prepare_frontend_gallery_html( saturnwp_prepare_gallery_attachments( $value, $id ), $order, $id, $args )); ?>
		</ul>
	    <?php
    }
}
 ...
 * @return array
 */
function saturnwp_prepare_gallery_attachments( $value, $post_id = null ){
	global $saturnwp_a13;
	...
	if ( ! empty( $value ) ) {
		...
		$proofing_enabled = $saturnwp_a13->saturnwp_get_meta( '_proofing', $post_id ) === 'on';
		...
		if ( $media_count ) {
			...
			saturnwp_prepare_external_media( $externals );
 ...
 */
function saturnwp_prepare_external_media(&$items){
	global $saturnwp_a13;

	$proofing_enabled = $saturnwp_a13->saturnwp_get_meta( '_proofing' ) === 'on';
 ...
 */
function saturnwp_prepare_frontend_gallery_html( $attachments, $order, $id, $args ){
	if( $order === 'DESC' ){
		...
		foreach ( $attachments as $item ) {
			...
			elseif( is_array($item) && $item['type'] === 'audio' ){
				...
				if ( $item['id'] === 'external' ) {
					...
					//video details
					$temp =  saturnwp_detect_movie( $item['videolink_link'] );
					...
					$collector[ 'video_player' ]   = saturnwp_get_movie_link( $temp );       //Video API address
					...
					list(
						...
						$collector[ 'thumb' ]
						) = saturnwp_get_gallery_item_images($item, $collector, $columns, $bricks_max_width, $brick_margin, $brick_proportion);
			...
			saturnwp_frontend_gallery_item_html( $collector, $id, $args );
 ...
 * @param array $args gallery arguments
 */
function saturnwp_frontend_gallery_item_html( $collector, $post_id, $args ){
	global $saturnwp_a13;
	static $id = 0;
	$id++;
	$proofing_enabled = $args['proofing'] === 'on';
	...
	if(strlen($collector[ 'filter_tags' ])){
		$album_filters = saturnwp_get_media_filters($post_id);
		$item_filters = array();
		$data_filter = '';
	...
	<li class="<?php echo esc_attr($collector[ 'item_class' ]); ?>"<?php echo esc_attr($data_attr); ?>>
		...
		if( $collector[ 'attachment_type' ] === 'internal' && $collector[ 'type' ] === 'video' ){
			...
			//saturnwp_video() produces escaped content
			echo wp_kses_post('<div class="album-video" id="album-video-'.esc_attr( $id ).'">'.saturnwp_video( $video_attr ).'</div>');
		}
		...
		if ( !$proofing_enabled ) {
			if(function_exists( 'ADDTOANY_SHARE_SAVE_KIT' )){
				...
				if ( $saturnwp_a13->get_option( 'album_social_icons' ) === 'on' && $sharing_enabled ){

					$back_to_post = $saturnwp_a13->get_option( 'album_share_type' ) === 'album';
					//share link will link to album and open image in lightbox
					if($back_to_post){
						$album_url = saturnwp_current_url();
...
//				    ADDTOANY_SHARE_SAVE_KIT( array( 'linkname' => $collector['title'], 'linkurl' => $collector['src'] ) );
		...
		//add to cart button
		if( saturnwp_is_woocommerce_activated() && isset( $collector['product_id'] ) && $collector[ 'product_id' ]){
 ...
 */
function saturnwp_calculate_height_proportion($brick_proportion){
	$height_proportion = 0;
	//prepare proportion from string
 ...
 */
function saturnwp_get_media_filters($id = null){
...
if ( ! function_exists( 'saturnwp_cpt_as_frontpage_title_fix' ) ) {
	/**
	 ...
	 */
	function saturnwp_cpt_as_frontpage_title_fix( $title ) {
...
if ( ! function_exists( 'saturnwp_cpt_as_frontpage_menu_fix' ) ) {
	/**
	 ...
	 */
	function saturnwp_cpt_as_frontpage_menu_fix( $classes, $item ) {
