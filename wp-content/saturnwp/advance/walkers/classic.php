<?php
/**
 * Class SATURNWP_menu_walker
 * Used in main menu
 */
class SATURNWP_menu_walker extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $hover_effect = $saturnwp_a13->get_option( 'menu_hover_effect' );
        $excluded_effect = in_array( $hover_effect, array('none','show_icon') );

        //check if it is vertical header and should sub-menu be open
        if( $saturnwp_a13->get_option( 'header_type' ) === 'vertical' &&
            $saturnwp_a13->get_option( 'submenu_active_open', 'off' ) === 'on' &&
        ...
        if($is_current_ancestor){
            $caret_class = 'fa-'.$saturnwp_a13->get_option( $depth > 0 ? 'submenu_third_lvl_closer' : 'submenu_closer' );
        }
        else{
            $caret_class = 'fa-'.$saturnwp_a13->get_option( $depth > 0 ? 'submenu_third_lvl_opener' : 'submenu_opener' );
        }
