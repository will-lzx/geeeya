...
function saturnwp__no_meta_posts_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             wp_kses(
                 /* translators: %1$s: link to customizer, %2$s section name */
                 __( 'Theme options for this page can be found in <a href="%1$s">Appearance -&gt; Customize -&gt; %2$s</a>.', 'saturnwp' ),
                 array(
                     'a'      => array(
                         'href' => array()
                     )
             ...
             esc_html__( 'Blog settings', 'saturnwp' )
         ) . '</p></div>';
}

function saturnwp__no_meta_shop_page_notice() {
    echo '<div class="notice notice-warning inline"><p>' .
         sprintf(
             wp_kses(
                /* translators: %1$s: link to customizer, %2$s section name */
             ...
             esc_html__( 'Shop(WooCommerce) settings', 'saturnwp' )
         ) . '</p></div>';
}
 ...
 */
function saturnwp_admin_meta_boxes(){
    global $saturnwp_a13;

    if(current_user_can( 'edit_posts' )){
        add_meta_box(
            'sktwb_theme_options',
             __( 'Blog post details', 'saturnwp' ),
            'saturnwp_meta_main_opts',
            'post',
            'normal',
            'default',
            array('func' => 'saturnwp_meta_boxes_post')//callback
        );
    }
    ...
    if(current_user_can( 'edit_pages' )){
        ...
        if($current_page_id == get_option( 'page_for_posts' )){
            add_meta_box(
                ...
                __( 'Page details', 'saturnwp' ),
                'saturnwp__no_meta_posts_page_notice',
                'page',
                'normal',
                'default'
            );
        ...
        //shop page
        elseif(saturnwp_is_woocommerce_activated() &&( $current_page_id == wc_get_page_id( 'shop' ) )){
            add_meta_box(
                'sktwb_theme_options_notice',
                __( 'Page details', 'saturnwp' ),
                'saturnwp__no_meta_shop_page_notice',
                'page',
                'normal',
                'default'
            );
            ...
            add_meta_box(
                ...
                'normal',
                'default',
                array('func' => 'saturnwp_meta_boxes_page')//callback
            );
        }
    }

}
add_action( 'add_meta_boxes', 'saturnwp_admin_meta_boxes');
 ...
 */
function saturnwp_meta_main_opts( $post, $meta_box ){

    // Use nonce for verification
    wp_nonce_field( 'sktwb_customization' , 'sktwb_noncename' );

    $input_prefix = SATURNWP_INPUT_PREFIX;
    ...
    global $pagenow, $saturnwp_a13;
    if('post-new.php' == $pagenow ) {
        foreach ( $meta_boxes as $meta_tab ) {
            foreach( $meta_tab as $meta ) {
                if( isset( $meta['id'] ) ){
                    $saturnwp_a13->defaults_of_meta[ $meta['id'] ] = isset( $meta['default'] ) ? $meta['default'] : '';
                }
            }
        }
            ...
            */
            ...
            if ( $meta['type'] === 'fieldset' ) {
                ...
                if( isset( $meta['companion_required'] ) && $meta['companion_required'] === true ){
                    saturnwp_is_companion_plugin_ready();
                }
            ...
            elseif( saturnwp_print_form_controls($meta, $params, true ) ){
                continue;
            }
            ...
            elseif ( $meta['type'] === 'multi-upload' ) { ?>
                ...
                <div class="a13-mu-container">
                    ...
                    echo esc_attr( __( 'Select/Upload images and videos', 'saturnwp' ) );
                    ?>"<?php echo ( isset( $meta['media_type'] ) && strlen( $meta['media_type'] ) ) ?
                        ' data-media-type="'.esc_attr($meta['media_type']) . '"' : ''; ?> />
                    <span class="button button-large add-link-media"><?php esc_html_e( 'Add Video from Youtube/Vimeo', 'saturnwp' ); ?></span>
                    <label class="button button-large"><input type="checkbox" id="mu-prepend" value="1" /><?php esc_html_e( 'Add items at beginning of the list', 'saturnwp' ); ?>
                    </label>
                    <input id="a13-multi-remove" class="button button-large" type="button" value="<?php echo esc_attr( __( 'Remove selected', 'saturnwp' ) ); ?>" disabled="disabled" />
                    <?php

                    do_action('saturnwp_a13_multi_upload_tools');

                    saturnwp_input_help_tip( __( 'To mark more items in Media Library and in below list, you can use <code>Ctrl</code>(<code>Cmd</code>) or <code>Shift</code> key while selecting them with mouse.', 'saturnwp' ));
                    ?>
                ...
                saturnwp_admin_gallery_item_html( 'attachment-preview image', 'thumbnail', get_theme_file_uri( 'images/holders/video_150x100.png') );
                echo '</div>';
                ?>
                <ul id="mu-media" class="media-frame-content" data-columns="5">
                    <?php saturnwp_prepare_admin_gallery_html( saturnwp_prepare_gallery_attachments( $value ) ); ?>
                </ul><?php
            }
        } //end foreach
    } //end foreach
 ...
 * @param int $post_id
 */
function saturnwp_save_post($post_id){
    static $done = 0;
    $done++;
    if( $done > 1 ){
        ...
        foreach ( $meta_boxes as $meta_tab ) {
            foreach( $meta_tab as $meta ) {
                ...
                if( isset( $meta['id'] ) && isset( $_POST[ $input_prefix . $meta['id'] ] ) ){
                    ...
                    update_post_meta( $post_id, '_' . $meta['id'], apply_filters( 'saturnwp_save_post_meta', $val, $meta['id'] ) );
                }
            }
        }
    }
}
add_action( 'save_post', 'saturnwp_save_post' );