...
function saturnwp_compare_dependency($dependency){
	global $saturnwp_a13;

	$parent   = $dependency[0];
	$operator = $dependency[1];
	$value    = $dependency[2];
	$parent_value = $saturnwp_a13->saturnwp_get_meta( '_'.$parent );

	//check if it is "new post" page
	global $pagenow;
	if('post-new.php' == $pagenow ) {
		$parent_value = $saturnwp_a13->defaults_of_meta[$parent];
	}
 ...
 * @return bool
 */
function saturnwp_check_control_dependencies($required, $is_meta ){
	if($is_meta){
		//we have more then one required condition
		if(is_array($required[0]) ){
			foreach($required as $dependency){
				if(!saturnwp_compare_dependency($dependency)){
		...
		//we have only one required condition
			...
			return saturnwp_compare_dependency($required);
		}
	}
...
function saturnwp_input_help_tip($message){
	?>
	<div class="input-tip">
 ...
 */
function saturnwp_print_form_controls( $option, &$params, $is_meta = false ) {
	global $saturnwp_a13;
	...
	if ( $is_meta ) {
		...
		$value = $saturnwp_a13->get_option( $option['id'] );
	}

	//check if field should be visible
	if ( isset( $option['required'] ) && is_array( $option['required'] ) ) {
		//display or not
		$style .= saturnwp_check_control_dependencies( $option['required'], $is_meta ) ? '' : 'display: none;';
	}
	...
	if ( $option['type'] == 'upload' ) {
		$upload_button_text = ! empty( $option['button_text'] ) ? $option['button_text'] : esc_html__( 'Upload', 'saturnwp' );
		?>
		...
		<div class="upload-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				?> />
				<input id="clear_<?php echo esc_attr($input_prefix . $option['id']); ?>" class="clear-image-button button" type="button" value="<?php echo esc_attr__( 'Clear field', 'saturnwp' ) ?>" />
	...
	elseif ( $option['type'] == 'color' ) {
		...
		<div class="color-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php saturnwp_input_help_tip( '<p>'.__( 'Use the correct <code>color</code> property in CSS( <code>green, #33FF99, rgb(255,128,0), rgba(222,112,12,0.5)</code> ), or choose a color with a color picker. Leave an empty value to use the default theme value.', 'saturnwp' ).'</p><p>'.__( 'Use the "Transparent" button to insert a transparent value.', 'saturnwp' ) ); ?>
				...
				<button class="transparent-value button-secondary"><?php esc_html_e( 'Transparent', 'saturnwp' ); ?></button>
				<p class="desc"><?php echo wp_kses( $description, $valid_tags ); ?></p>
			</div>
		</div>
		<?php
		...
		<div class="tag_media-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				<p><input id="<?php echo esc_attr($input_prefix . $option['id']); ?>-helper" class="not-to-collect newtag" size="16" value="" type="text">
					<input class="not-to-collect button tagadd" value="<?php esc_attr_e( 'Add', 'saturnwp' ); ?>" type="button"></p>
	...
	elseif ( $option['type'] == 'slider' ) {
		...
		<div class="slider-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				<?php saturnwp_input_help_tip( __( 'Use slider to set proper value. You can click on slider handle and then use arrows keys(on keyboard) to adjust value precisely. You can also type in input value that is in/out of range of slider, and it will be used.', 'saturnwp' ) ); ?>
		...
		<div class="select-input input-parent"<?php echo strlen($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
			...
			<div class="input-desc">
				...
				if ( $products ) {
					...
					echo '<option value="0">'.esc_html__( 'None', 'saturnwp' ).'</option>';
					...
					echo '<span class="empty-type">'. esc_html__( 'There are no products yet!', 'saturnwp' ) . '</span>';
				}
