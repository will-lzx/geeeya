...
(function($){
    ...
    window.SATURNWP_ADMIN = {
        ...
        onReady : function(){
            ...
            SATURNWP_ADMIN.upload();
            SATURNWP_ADMIN.utils.init();
            SATURNWP_ADMIN.metaActions.init();
            SATURNWP_ADMIN.settingsAction();
		},
...
//                                    current_input.focus();
                            ...
                            if (items_num) {
								...
								if(to_send_array.length){
									$.ajax({
										...
										url: ajaxurl,
										data: {
											action : 'saturnwp_prepare_gallery_items_html', //called in backend
											items : to_send_array
										},
                        ...
                        updateItem = function(){
							...
							if(item.id === 'external'){
								...
								$.ajax({
									...
									url: ajaxurl,
									data: {
										action : 'saturnwp_prepare_gallery_single_item_html', //called in backend
										item : item
									},
									success: function(new_html) {
										//insert HTML
                        ...
                        if(dependency.length){
                            ...
                            if(Array.isArray(dependency[0])){
                                for(var i = 0; i < dependency.length; i++ ){
                                    if(!SATURNWP_ADMIN.metaActions.compareDependency(dependency[i], possible_switches)){
                            ...
                            //we have only one required condition
                                ...
                                if(!SATURNWP_ADMIN.metaActions.compareDependency(dependency, possible_switches)){
        ...
        settingsAction : function(){
            ...
            SATURNWP_ADMIN.proofingNotice();
            SATURNWP_ADMIN.adminNotices();
			...
			$('#vc_ui-panel-edit-element').on('click','.a13_delete_selected_nava', function(){
                ...
                if(confirm(G.messages.confirm_delete_nava)){
                    $.ajax({
                        ...
                        data: {
                            action: 'saturnwp_nava_delete_post',
                            id: id
                        },
            ...
            if(proofing_notice.length){
                $('#hide-proof-notice').on('click', function(e){
                    ...
                    $.ajax({
                        ...
                        url: ajaxurl,
                        data: {
                            action : 'saturnwp_proofing_notice_action' //called in backend
                        },
            ...
            if(notices.length){
                notices.on('click', '.notice-dismiss', function(e){
                    ...
                    $.ajax({
                        ...
                        url: G.ajaxurl,
                        data: {
                            action : 'saturnwp_disable_ajax_notice', //called in backend
    ...
    var SATURNWP_ADMIN = window.SATURNWP_ADMIN;

    //start ADMIN
    $(document).ready(SATURNWP_ADMIN.onReady);
