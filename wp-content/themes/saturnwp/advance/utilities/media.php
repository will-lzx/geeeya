...
if(!function_exists('saturnwp_get_top_image_video')) {
	/**
	 ...
	 */
	function saturnwp_get_top_image_video( $link_it = false, $args = '' ) {
        global $saturnwp_a13;
        ...
        if(
            ($is_post && $saturnwp_a13->get_option( 'post_media') == 'off')
            ||
            ($is_post_list && $saturnwp_a13->get_option( 'blog_media') == 'off')
        ){
            return $html; //empty string
        }
        ...
        $image_video    = $saturnwp_a13->get_option( 'blog_videos') === 'off' && ($is_post_list || $is_shortcode);

        $thumb_size = 'skt-post-thumb'; //default for post

        if( ! $is_shortcode && ( $is_page || $is_post || $is_work ) ){
            $layout = $is_page? $saturnwp_a13->saturnwp_get_meta('_content_layout', $post_id) : $saturnwp_a13->get_option( 'post_content_layout');
            $full_layouts =  array(
                'full_padding',
                'full',
            );
            ...
            elseif(in_array($layout, $small_layouts)){
                ...
                if( defined('SATURNWP_NO_SIDEBARS') || $saturnwp_a13->saturnwp_get_meta( '_widget_area' ) == 'off'){
                    $thumb_size = 'skt-post-thumb-big';
                }
            }
        }
        elseif( $is_post_list || $is_shortcode ){
            if($saturnwp_a13->get_option( 'blog_post_look') === 'horizontal'){
                $thumb_size = 'skt-blog-posts-horizontal';
            }
            else{
                $thumb_size = 'skt-blog';
                $brick_size         = $saturnwp_a13->saturnwp_get_meta('_brick_ratio_x', $post_id);
                $columns            = array_key_exists( 'columns', $args ) ? $args['columns'] : (int)$saturnwp_a13->get_option( 'blog_brick_columns' );
                $bricks_max_width   = array_key_exists( 'max_width', $args ) ? $args['max_width'] : (int)$saturnwp_a13->get_option( 'blog_bricks_max_width' );
                $brick_margin       = array_key_exists( 'margin', $args ) ? $args['margin'] : (int)$saturnwp_a13->get_option( 'blog_bricks_margin' );
                ...
                if( $saturnwp_a13->saturnwp_get_meta( '_image_parallax' ) !== 'on' ){
                    $image_proportion =  $saturnwp_a13->get_option( 'blog_bricks_proportions_size', '0' );
                    $height_proportion =  saturnwp_calculate_height_proportion($image_proportion);
        ...
        if( $args['force_image'] || $img_or_vid === 'post_image' ){
            $is_parallax = $saturnwp_a13->saturnwp_get_meta( '_image_parallax' ) === 'on';
            if($args['return_src']){
                $html = saturnwp_make_post_image($post_id, $sizes[$thumb_size], true);
            }
            elseif($is_parallax){
	            $img_src = saturnwp_make_post_image($post_id, $sizes[$thumb_size], true);
                if($img_src !== false ){
                    $parallax_height = $saturnwp_a13->saturnwp_get_meta('_image_parallax_height', $post_id);
                ...
                $additional_class = '';
                $src = saturnwp_make_post_image( $post_id, $sizes['full'], true );
                ...
                if( $file_type['type'] === 'image/gif'){
                    $img = saturnwp_make_post_image($post_id, $sizes['full']);
                    $additional_class = ' animated-gif';
                }
                else{
                    $img = saturnwp_make_post_image($post_id, $sizes[$thumb_size]);
                }
        ...
        elseif( $img_or_vid === 'post_video' ){
            ...
            if($image_video){
	            $html = saturnwp_get_top_image_video($link_it, array_merge($args, array('force_image' => true )));
            }
            else{
                ...
                if( !empty( $src ) ){
                    ...
                    if($v_code === false){
                        $video_data  = saturnwp_detect_movie($src);
...
if(!function_exists('saturnwp_top_image_video')){
    /**
     ...
     *
     */
	function saturnwp_top_image_video($link_it = false, $args = ''){
        //saturnwp_get_top_image_video() produces escaped content
        echo wp_kses_post(saturnwp_get_top_image_video($link_it, $args));
    }
}


if(!function_exists('saturnwp_make_post_image')){
	/**
	 ...
	 */
	function saturnwp_make_post_image( $post_id, $sizes, $only_src = false ){
        global $saturnwp_a13;
...
if(!function_exists('saturnwp_detect_movie')){
	/**
	 ...
	 */
	function saturnwp_detect_movie($src){
...
if(!function_exists('saturnwp_get_movie_link')){
	/**
	 * Returns movie link to for video API
	 *
	 * @param array $video_data data received from saturnwp_detect_movie function
	 *
	 * @return bool|string API URL if video type is matched, false otherwise
	 */
	function saturnwp_get_movie_link( $video_data ){
 ...
 */
function saturnwp_video( $attr, $dont_load_video_library = false ) {
	global $content_width, $saturnwp_a13;
	$post_id = 0;
 ...
 * @return float ration
 */
function saturnwp_get_svg_height_ratio($svg){
