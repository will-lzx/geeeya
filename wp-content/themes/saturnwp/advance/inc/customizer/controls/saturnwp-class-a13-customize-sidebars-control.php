 ...
 *
 */
class SaturnWp_A13_Customize_Sidebars_Control extends WP_Customize_Control {
	 ...
	 *
	 * @see SaturnWp_A13_Customize_Socials_Control::content_template()
	 */
		...
		<div class="customize-control-content">
			...
			#>
				<div class="custom_sidebar-row clearfix">
					<label class="screen-reader-tex" for="{{ id_prefix }}sidebar-{{ index }}"><?php esc_html_e( 'Name of sidebar', 'saturnwp' ); ?> {{ index }}</label>
					<input type="text" class="custom_sidebar" name="{{ index }}" id="{{ id_prefix }}sidebar-{{ index }}" value="{{ value }}" />
					<button class="remove"><?php esc_html_e( 'Remove this sidebar', 'saturnwp' ); ?></button>
				</div>
			<# } ); #>
			<button class="button button-primary add-new"><?php esc_html_e( 'Add sidebar', 'saturnwp' ); ?></button>
