 ...
 * Access to this singleton is via global $saturnwp_a13
 */
class SaturnWp_Framework
{
     ...
     * kind of constructor
     */
    function saturnwp_start()
    {
        /**
         * Define bunch of helpful paths and settings
         */
        define('SATURNWP_TPL_SLUG', 'saturnwp');//it is not always same as directory of theme
        define('SATURNWP_OPTIONS_NAME_PART', 'saturnwp');
        define('SATURNWP_THEME_ID_NUMBER', '66');
        define('SATURNWP_OPTIONS_NAME', 'skt13_option_saturnwp');
        define('SATURNWP_CACHE', 'skt13_saturnwp_cache');
        define('SATURNWP_THEME_VERSION', '1.0');
        define('SATURNWP_THEME_VER', SATURNWP_THEME_VERSION ); //legacy - do not use
        define('SATURNWP_MIN_COMPANION_VERSION', '1.0');
        define('SATURNWP_MIN_PHP_VERSION', '5.6');
        define('SATURNWP_MIN_WP_VERSION', '5.0');

        //theme root
        define('SATURNWP_TPL_URI', get_template_directory_uri());

        //plugins recommended by theme
        define('SATURNWP_TPL_PLUGINS', SATURNWP_TPL_URI . '/advance/plugins');
        define('SATURNWP_TPL_PLUGINS_DIR', get_template_directory() . '/advance/plugins');

        //misc theme globals
        define('SATURNWP_INPUT_PREFIX', 'a13_');
        define('SATURNWP_CONTENT_WIDTH', 800);

        //check minimal requirements for WordPress and PHP
        if (
            version_compare( $GLOBALS['wp_version'], SATURNWP_MIN_WP_VERSION, '<' )
            ||
            version_compare( PHP_VERSION, SATURNWP_MIN_PHP_VERSION, '<' )
        ) {
        ...
        if( is_customize_preview() ){
            ...
            add_action( 'wp_loaded', array( $this, 'saturnwp_customizer_wp_loaded' ) );
            //before save
            add_action('customize_save', array($this, 'saturnwp_customizer_customize_save_before'));
            //perform option save while using customizer
            add_action('customize_save_after', array($this, 'saturnwp_customizer_customize_save_after'));
        }
        ...
        if ( is_admin() ) {
            ...
            /** @noinspection PhpIncludeInspection */
			get_template_part('advance/admin/saturnwpinfo', 'pages');
            /** @noinspection PhpIncludeInspection */
			
			get_template_part('advance/admin/saturnwpinfo-pages-functions');
            ...
            if (current_user_can('update_core')) {
                add_action( 'admin_notices', array(&$this, 'saturnwp_check_for_warnings') );
            }
        }
        ...
        get_template_part('advance/saturnwp');  
        ...
        add_action('after_setup_theme', array(&$this, 'saturnwp_setup'));
    }
     ...
     */
    function saturnwp_set_sections($section){
        /**
         * @since 2.3.0
         */
        do_action( 'saturnwp_options_before_'.$section['id'] );
         ...
         * @since 2.3.0
         */
        do_action( 'saturnwp_options_after_'.$section['id'] );
    }
     ...
     */
    function saturnwp_get_sections(){
     ...
     * @return array
     */
    function saturnwp_set_settings_set($set, $values){
     ...
     * @since 2.3.0
     */
    function saturnwp_get_settings_set($set){
     ...
     */
    function saturnwp_customizer_wp_loaded() {
        $this->theme_options = get_option(SATURNWP_OPTIONS_NAME);
        $this->load_options();
    }
     ...
     */
    function saturnwp_customizer_customize_save_before()
    {
     ...
     */
    function saturnwp_customizer_customize_save_after()
    {
        //get new set of options
        $this->theme_options = get_option(SATURNWP_OPTIONS_NAME);
        $this->load_options();

        do_action( 'saturnwp_generate_user_css' );

        //refresh cache
        delete_option(SATURNWP_CACHE);
    }

    /**
     * Various setup actions for setting up theme for WordPress
     */
    function saturnwp_setup()
    {
        global $content_width;
        //content width
        if (!isset($content_width)) {
            $content_width = SATURNWP_CONTENT_WIDTH;
        }
        ...
        if (
            ...
            ( function_exists('saturnwp-a13fe_user_css_name') && ! file_exists( saturnwp-a13fe_user_css_name() ) ) ||
        ...
        //LANGUAGE
        load_theme_textdomain( 'saturnwp', get_theme_file_path( 'languages' ) );
        ...
        register_nav_menus(array(
            'header-menu' => __( 'Site Navigation', 'saturnwp' ),
        ));
    }
     ...
     */
    function saturnwp_check_for_warnings()
    {
        $notices = array();
        $valid_tags = array(
            'a' => array(
        ...
        foreach ($notices as $id => $notice) {
            ...
            if( !saturnwp_is_admin_notice_active($id) ){
                continue;
            }
        ...
        do_action( 'saturnwp_theme_notices' );
    }
    ...
    public function set_options( $overload_options = array() )
    {
        if( is_array($overload_options) && count($overload_options) > 0){
            update_option(SATURNWP_OPTIONS_NAME, $overload_options);
    ...
    public function get_option($index, $default = '', $filter = true)
    {
        ...
        return apply_filters('saturnwp_a13_options_'.$index, $option_to_return );
    }
    ...
    public function get_option_media_url($index)
    {
        ...
        elseif( is_string($option) && ( strlen($option) > 0 ) ){
            if(strncmp($option, "http", 4) !== 0){
                ...
                $option = SATURNWP_TPL_URI . $option;
            }
        }
    ...
    public function get_option_color_rgba( $index )
    {
        ...
        if ( is_array( $option ) ) {
            if ( isset( $option['rgba'] ) ) {
                ...
                return saturnwp_hex2rgba( $option['color'], $option['alpha'] ); //we got RGBA
            } else {
                return ''; //empty string as it is probably not set yet
            }
        }
...
//            'images_manager' //no parent options here
        ...
        foreach ($option_func as $function) {
            $function_to_call = 'saturnwp_meta_boxes_' . $function;
    ...
    public function get_meta_required_array() {
        ...
        if(strlen($post_type)){
            switch ( $post_type ) {
                case 'post':
                    $meta_boxes = saturnwp_meta_boxes_post();
                    break;
                case 'page':
                    $meta_boxes = saturnwp_meta_boxes_page();
                    break;
            }
    ...
    public function prepare_theme_vars(){
        $cache = get_option( SATURNWP_CACHE );
        ...
        elseif( $cache === false || $cache['version'] !== SATURNWP_THEME_VERSION || defined( 'ICL_SITEPRESS_VERSION') || defined( 'POLYLANG_BASENAME' ) ){
            ...
            //normal flow
                ...
                $cache = array(
                    ...
                    'version' => SATURNWP_THEME_VERSION
                );

                //save cache
                update_option( SATURNWP_CACHE, $cache );
            }
        }
        //cache hit
        else{
     ...
     */
    function saturnwp_get_meta($field, $id = false)
    {
        $family = '';

        if (!$id && saturnwp_is_no_property_page()) {
     ...
     */
    function saturnwp_get_social_icons_list($what = 'names'){
        $icons = array(
        ...
        $icons = apply_filters('saturnwp_social_icons_list', $icons );
        $result = array();
    ...
    function saturnwp_get_standard_fonts_list(){
        return array(
    ...
    function saturnwp_is_companion_plugin_ready($fail_message = false, $silent = false){
        ...
        if ( empty( $plugins[$plugin_file] ) ) {
            ...
            $label = __( 'Install', 'saturnwp' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'saturnwp' );
        }
        ...
        elseif ( is_plugin_inactive( $plugin_path ) ){
            ...
            $label = __( 'Activate', 'saturnwp' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'saturnwp' );
        }

        //not up to date
        elseif( version_compare( $plugins[$plugin_file]['Version'], SATURNWP_MIN_COMPANION_VERSION, '<'  ) ){
            $ready = false;
            wp_enqueue_script( 'updates' );
            $classes =  ' update-now';
            ...
            $label = __( 'Update', 'saturnwp' ) . ' : ' . __( 'SKT Templates – Elementor & Gutenberg templates', 'saturnwp' );
        }

        if(!$ready && !$silent){
            $message = $fail_message ? $fail_message : __( 'This feature requires SKT Templates plugin to be active and in the proper version.', 'saturnwp' );
