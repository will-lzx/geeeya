...
add_action( 'after_switch_theme', 'saturnwp_switch_theme'   );
add_action( 'load-customize.php', 'saturnwp_load_customize' );
add_action( 'template_redirect',  'saturnwp_preview'        );
 ...
 * @return string
 */
function saturnwp_compat_message() {
	if ( version_compare( $GLOBALS['wp_version'], SATURNWP_MIN_WP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: the required WordPress version, %3$s: user's current version */
			__( '%1$s theme requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'saturnwp' ),
			SATURNWP_OPTIONS_NAME_PART,
			SATURNWP_MIN_WP_VERSION,
			$GLOBALS['wp_version']
		);

	} elseif ( version_compare( PHP_VERSION, SATURNWP_MIN_PHP_VERSION, '<' ) ) {

		return sprintf(
		/* translators: %1$s: Theme name, %2$s: required PHP version, %3$s: current PHP version */
			__( '%1$s theme requires at least PHP version %2$s. You are running version %3$s. Please upgrade and try again.', 'saturnwp' ),
			SATURNWP_OPTIONS_NAME_PART,
			SATURNWP_MIN_PHP_VERSION,
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function saturnwp_switch_theme( $old_name ) {
 ...
 * @return void
 */
function saturnwp_load_customize() {

	wp_die( esc_html( saturnwp_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function saturnwp_preview() {

	if ( isset( $_GET['preview'] ) ) { // WPCS: CSRF ok.
		wp_die( esc_html( saturnwp_compat_message() ) );
	}
}
